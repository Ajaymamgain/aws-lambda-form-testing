...
import { Schedule, ScheduleTestRequest } from "@/types/api";
...
export default function EditSchedulePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      ...
      try {
        ...
        // Set success indicator visibility
        if (mockSchedule.testConfig?.formConfig.successIndicator) {
          setShowSuccessIndicator(true);
        }
        
        // Find matching cron preset
        const matchingPreset = cronPresets.find(preset => preset.value === mockSchedule.cronExpression);
        if (matchingPreset) {
        ...
        reset({
          ...
          testConfig: mockSchedule.testConfig || {
            url: "",
            formConfig: {
              fields: [],
              submitButtonSelector: "",
          ...
          active: mockSchedule.active !== undefined ? mockSchedule.active : true,
        });
      } catch (error) {
  ...
  const onSubmit = async (data: ScheduleFormData) => {
    ...
    try {
      // In a real implementation, this would call the API
      // await api.schedules.update(params.id, data);
      ...
      // Redirect back to schedule detail page
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-neutral-900">Edit Schedule</h1>
          <p className="mt-1 text-neutral-500">
            Update your recurring test configuration
          </p>
        </div>
      </div>
